...
const EditTrip = () => {
	...
	const handlerAddMember = async () => {
		...
		let members = await AsyncStorage.getItem("tripMembers");
		...
		await AsyncStorage.setItem("tripMembers", JSON.stringify(updatedTripers));
	};

	React.useEffect(() => {
		const changeUser = async () => {
			let users = await AsyncStorage.getItem("tripMembers");
	...
	const handlerDeleteInput = async (id) => {
		...
		if (members) {
			...
			await AsyncStorage.setItem("tripMembers", JSON.stringify(updateMembers));
			setTripMember(updateMembers)
		}

	}
	const handlerSaveButton = async () => {
		await AsyncStorage.setItem("tripName", JSON.stringify(editTrip));
		await AsyncStorage.setItem("tripMembers", JSON.stringify(tripMember));

		navigation.navigate('trip/dashboard');
