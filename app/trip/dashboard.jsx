...
const ExpensePage = () => {
	...
	const handlerSavebutton = async () => {
		...
		if (newUser.name && newUser.cost > 0 && newUser.purpose) {
			try {
				let values = await AsyncStorage.getItem("accountList");
				...
				await AsyncStorage.setItem("accountList", JSON.stringify(updatedTripers));
	...
	React.useEffect(() => {
		...
		const loadAccountList = async () => {
			try {
				const stored = await AsyncStorage.getItem("accountList");
				if (stored) {
	...
	const handlerCalculateButton = () => {
		navigation.navigate('trip/result', {
			allMember: triper,
			prevMember: persons,
			userList: triper
		});
	...
	React.useEffect(() => {
		const getValue = async () => {
			const storedValueTrimMembers = await AsyncStorage.getItem("tripMembers");
			const storedValueTitle = await AsyncStorage.getItem("tripName");
				...
				<View style={[styles.card, { marginTop: 20, marginLeft: 8.5 }]}>
					...
					<View style={{
						...
						<View style={{ flexDirection: "row", justifyContent: "space-between", marginVertical: 7 }}>
							<Text style={{ color: "#2C4271", fontSize: 14, fontFamily: "Poppins_400Regular", overflow: 'hidden', marginTop: 2 }}>Trip Total Cost: <Text style={{ color: "#DA2C35" }}>${sum > 999999 ? sum.toString()?.slice(0, 4) + ".." : sum}</Text></Text>
